...
<template>
    <div class="form-panel">
        <p class="hint-text" v-show="data.list.length === 0">
            <a-empty description="从左侧选择组件添加" />
        </p>

        <a-form :form="form" :hideRequiredMark="data.config.hideRequiredMark" :layout="data.config.layout" :style="data.config.customStyle" class="a-form-box mor-form-build">
            <draggable
                @add="deepClone"
            ...
            >
                <transition-group class="list-main" name="list" tag="div">
                    <layoutItem
                        ...
                        :startType="startType"
                        :style="record.type == 'group' ? 'width:100%' : ''"
                        @dragStart="dragStart"
        ...
        <!-- 右键菜单 start -->
        <!-- <div :style="{ top: menuTop + 'px', left: menuLeft + 'px' }" class="right-menu" v-show="showRightMenu">
            <ul>
...
export default {
  ...
  methods: {
    ...
    handleColAdd(evt, copyItem, isCopy = false) {
      const model = copyItem.model;
      const key = copyItem.type + "_" + new Date().getTime();
      if (copyItem.key === "" || isCopy) {
        this.$set(copyItem, "key", key);
        if (copyItem.type == "group")
          if (model) {
            const index = model.indexOf("_");
            if (index > -1) {
              const count = model.substr(index);
              const str = model.substr(0, index);
              this.$set(copyItem, "model", str + (count + 1 + ""));
            } else {
              this.$set(copyItem, "model", key);
            }
          } else {
            this.$set(copyItem, "model", key);
          }
      ...
      // this.$emit("handleSetSelectItem", copyItem);
      // 重置或者生成key值
      // const newIndex = evt.newIndex;
        ...
        array.forEach((element, index) => {
          ...
          if (element.key === this.selectItem.key) {
            if (isCopy) {
              ...
              this.handleColAdd(evt, copyItem, true);
              array.splice(index + 1, 0, copyItem);
              //深拷贝
              array = JSON.parse(JSON.stringify(array));
              this.$emit("handleSetSelectItem", array[index + 1]);
            } else {
              // 双击添加到选择节点后面
              ...
              this.handleColAdd(evt, copyData, true);
              array.splice(index + 1, 0, copyData);
            }
            return;
          }
          ...
          } else if (element.type === "card" || element.type === "group") {
            // 卡片布局
            if (element.list && element.list.length > 0) traverse(element.list);
